...
public class SwiftGeofirePlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channelAlpha = FlutterMethodChannel(name: "geofireAlpha", binaryMessenger: registrar.messenger())
        let eventChannelAlpha = FlutterEventChannel(name: "geofireStreamAlpha",
                                               binaryMessenger: registrar.messenger())
        
        let channelBeta = FlutterMethodChannel(name: "geofireBeta", binaryMessenger: registrar.messenger())
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if(call.method.elementsEqual("GeoFire.start")){
            ...
            geoFireRef = Database.database(url: nonDefaultUrl).reference().child(path)
            geoFire = GeoFire(firebaseRef: geoFireRef!)
            result(true)
        }
            ...
            if(circleQuery != nil){
                ...
                self.circleQuery?.center = CLLocation(latitude: lat, longitude: lng)
